...
class IntArray():
    ...
    def insert(self, index: int, val: int) -> None:
        ...
        '''
        This function uses the logic of inset by dividing the array into two equal half before the index and after the index 
        first it copy the element of the array into the new memory and copy the rest element leaving the memory to insert the given val into
        given index 
        To copy first part O(index) +O(size of array - index) = O(n) + C = O(n)
        '''
        if not isinstance(val, int) or not self._min_val <= val <= self._max_val:
            raise TypeError(f'Value must be an integer between {self._min_val} and {self._max_val}')

        #increasing the memory allocated
        new_size = self._size + 1

        # allocationg new memory
        ...
        #copying elements after the insert index from old memory
        ...
        #sets new value at the insert index
        self._resmem = new_resmem
        #assigning new size for inserting at the given index
        self._size = new_size
        #inserting the given val in given index
    ...
    def remove(self,index:int):
        ...
        #decreasing the memory allocated
        new_size = self._size -1

        # allocationg new memory
        new_resmem = ReservedMemory(new_size * self._bytes_per_element)
        
        #getting the value of the array at given index
        index_val = self.__getitem__(index)
        ...
        #copying elements after the removing index from old memory
        if self._size - index > 0:
            new_resmem.copy(
                self._resmem,
